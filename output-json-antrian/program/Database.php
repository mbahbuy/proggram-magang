...
$date = date( 'Y-m-d' );
$textantrian = poli( $data['pasien_need'] ); 
    ...
    if( $data['pasien_nik'] !== '' )
    {
        if( $data['pasien_need'] !== '' )
        {
            ...
            } elseif( date( 'H:i:s' ) < "04:00:00" )
            {
                ...
                $sqlcheck = $conn->query( "SELECT * FROM book WHERE ( tanggal LIKE '%$date%' AND book_payment = $data[pasien_nik] AND produk_id = $data[pasien_need] ) ");
                // echo "SELECT * FROM book WHERE ( tanggal LIKE '%$date%' AND book_payment = $data[pasien_nik] AND produk_id = $data[pasien_need] ) ";die;
                if( $sqlcheck->num_rows > 0 )
                {
                    ...
                    // echo 'tidak ada';die;
                    $dataAntrian = $conn->query( "SELECT book_timer FROM book WHERE ( tanggal LIKE '%$date%' AND produk_id = $data[pasien_need] ) ORDER BY book_timer DESC LIMIT 1 " );
